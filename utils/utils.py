...
FCVPT_datasets = ['DescribableTextures',  'EuroSAT', 'OxfordFlowers', 'SUN397', 'UCF101', 'ImageNetR', 'ImageNetSketch',
                  'ImageNetA', 'CIFAR10_local', 'CIFAR100_local', 'ImageNet', 'Caltech101',
                  'FGVCAircraft', 'Food101', 'OxfordPets']
...
def setup_text_training_utils(args, model):
    ...
    for key, value in model.named_parameters():
        if 'pcl_student' in key and 'pcl_teacher' not in key:
...
def setup_training_utils(args, model):
    model = model.cuda()
    model = model.float()
    params = list()
    for key, value in model.named_parameters():
        if key == 'prompt_embeddings':
            value.requires_grad = True
        elif 'pcl_student' in key and 'pcl_teacher' not in key:
            value.requires_grad = True
...
def test_prompting(teloader, model):
    ...
    for i, inputs in enumerate(tqdm(teloader)):
        ...
        with torch.no_grad():
            ...
            outputs, _ = model.eval_clip(inputs)
...
def CLIP_Inference(model, loader):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(tqdm(loader)):
            ...
            images = images.cuda()
            target = target.cuda()
            out, img_feas = model(images)
            all_img_feas.append(img_feas)
...
def test(teloader, model):
    ...
    for i, inputs in enumerate(tqdm(teloader)):
        ...
        with torch.no_grad():
            inputs, labels = inputs.cuda(), labels.cuda()
            outputs, img_feas = model.eval_clip(inputs)
            _, predicted = outputs.max(1)
            losses.append(criterion(outputs, labels).cpu())
            one_hot.append(predicted.eq(labels).cpu())
            all_img_feas.append(img_feas)
